...
static bool
CheckFleeingPoint( const gentity_t *self, const botTarget_t target )
{
	vec3_t navmeshPoint; // point to check
	...
	recast2quake( navmeshPoint );

	navmeshPoint[2] = destination[2]; // only count the distance in x, y

	if ( DistanceSquared( &destination[0], navmeshPoint ) > Square(50.0f) ) {
		juggernautLogger.Debug("Flee point is too far from a navmesh");
		return false;
	}

	if ( DistanceSquared( self->s.origin, navmeshPoint ) < Square(30.0f) ) {
